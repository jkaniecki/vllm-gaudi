...
def hpu_chunk_gated_delta_rule(
    ...
    # Stage 1: per-chunk cumulative g in log-space.
    ...
    for seq_id, (bos, eos) in enumerate(seq_ranges):
        ...
        for cs in range(bos, eos, chunk_size):
            ...
            if use_vectorized_chunk:
                ...
                k_h = k_chunk.permute(1, 0, 2).contiguous()
                g_h = g_chunk.transpose(0, 1).contiguous()
                beta_h = beta_chunk.transpose(0, 1).contiguous()
